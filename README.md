...
the values that occur. Columns can be hidden and reordered programmatically and saved with a
layout, but the header has no drag handle and there is no built-in column chooser. There is also no
export, no tree tables, and no accessibility semantics. Frozen columns pin left only. The
